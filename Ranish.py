...
# -*- coding: utf-8 -*-
"""
ربات بله – نسخهٔ ۲۱ (PulseAudio Pro)
رفع قطعی ضبط صدا با PulseAudio null-sink، ارسال مستقیم ویدئو،
حذف callbackهای انتخاب نحوهٔ دریافت، افزودن video_delivery به تنظیمات،
و لاگ‌گیری برای دیباگ.
"""
...
# صف جداگانه برای ضبط ویدیو
record_queue = queue.Queue()
record_queue_lock = threading.Lock()

def debug_log(msg: str):
    """لاگ ساده برای دیباگ صدای ضبط"""
    try:
...
class UserSettings:
    ...
    video_format: str = "webm"          # "webm", "mkv", "mp4"
    incognito_mode: bool = False
    video_delivery: str = "split"       # "split" یا "zip" ★ جدید
...
def get_session(chat_id):
    ...
    if key in data:
        ...
        for k, v in d.items():
            ...
            elif k in ("ad_blocked_domains", "found_downloads", "last_settings_msg_id"):
                setattr(s, k, v)
            else: setattr(s, k, v)
...
# ═══════════════════════ منوها (تغییر: video_delivery اضافه شد) ═══════════════════════
...
def settings_keyboard(settings: UserSettings):
    ...
    return {"inline_keyboard": [
        ...
        [{"text": f"📦 ارسال ویدیو: {delivery}", "callback_data": "set_viddel"}],
...
# ═══════════════════════ ابزارهای صوتی (PulseAudio null-sink) ═══════════════════════
def has_audio_support() -> bool:
    return shutil.which("ffmpeg") is not None

def setup_pulseaudio_virtual():
    """راه‌اندازی PulseAudio و یک بلندگوی مجازی (null-sink)"""
    try:
        # نصب pulseaudio در صورت نبود
        if shutil.which("pulseaudio") is None:
            ...
            subprocess.run(["apt-get", "install", "-y", "-qq", "pulseaudio"], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        subprocess.run(["pulseaudio", "--start"], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        time.sleep(1)
        # ایجاد null sink (بلندگوی مجازی)
        result = subprocess.run(
            ["pactl", "load-module", "module-null-sink", "sink_name=virtual_out", "sink_properties=device.description=Virtual_Output"],
            capture_output=True, text=True
        )
        if result.returncode == 0:
            safe_print("PulseAudio null-sink virtual_out loaded")
            os.environ["PULSE_SINK"] = "virtual_out"
            debug_log("PulseAudio virtual_out created and PULSE_SINK set")
            return True
        else:
            safe_print("pactl load-module failed, trying fallback")
            # اگه null-sink قبلاً لود شده باشه، تلاش برای استفاده ازش
            os.environ["PULSE_SINK"] = "virtual_out"
            return True
    except Exception as e:
        safe_print(f"PulseAudio setup failed: {e}")
        debug_log(f"PulseAudio setup error: {e}")
        return False

def start_audio_capture_virtual(job_dir: str) -> Tuple[Optional[subprocess.Popen], str]:
    ...
    try:
        ...
        debug_log(f"started ffmpeg pulse capture to {audio_path}")
        return proc, audio_path
    except Exception as e:
        debug_log(f"start_audio_capture_virtual failed: {e}")
        return None, audio_path
...
def stop_audio_capture(proc: Optional[subprocess.Popen], audio_path: str) -> bool:
    ...
    try:
        ...
        debug_log(f"audio capture stopped, size={size} bytes")
        return size > 0
    except Exception as e:
        debug_log(f"stop_audio_capture error: {e}")
        return False

# ═══════════════════════ استخراج المان‌ها (سه حالته) – بدون تغییر ═══════════════════════
def extract_clickable_and_media(page, mode="text"):
    # (کد موجود عیناً حفظ می‌شود)
    if mode == "text":
        raw = page.evaluate("""() => {
...
# ═══════════════════════ هستهٔ پردازش Job (عمومی) – بدون تغییر ═══════════════════════
...
# ═══════════════════════ ضبط ویدیو (بازنویسی با PulseAudio + ارسال مستقیم) ═══════════════════════
def handle_record_video(job):
    ...
    delivery = session.settings.video_delivery  # "split" یا "zip"
    job_dir = os.path.join("jobs_data", job.job_id)
    os.makedirs(job_dir, exist_ok=True)
    ...
    try:
        # ★ راه‌اندازی PulseAudio فقط در صورت نیاز
        pulse_ok = False
        if audio_enabled:
            pulse_ok = setup_pulseaudio_virtual()
            if pulse_ok:
                audio_proc, audio_path = start_audio_capture_virtual(job_dir)
        ...
        try:
            ...
            # رفتارهای ضبط – کاملاً بدون تغییر
        ...
        # ★ ارسال مستقیم فایل‌ها با توجه به تنظیمات delivery
...
# ═══════════════════════ پارت اول تا اینجا ادامه دارد... ═══════════════════════
# برای دریافت ادامه فایل، لطفاً "پارت دوم" را درخواست کنید.


# ═══════════════════════ مرورگر (سه حالته + ناشناس) – بدون تغییر ═══════════════════════
...
# ═══════════════════════ تحلیل هوشمند (Explorer) – بدون تغییر ═══════════════════════
...
# ═══════════════════════ تحلیل سورس (Explorer) – بدون تغییر ═══════════════════════
...
# ═══════════════════════ جستجوی فایل‌ها (سه مرحله‌ای) – بدون تغییر ═══════════════════════
...
def handle_callback(cq):
    ...
    if data == "menu_screenshot":
        ...
        send_message(chat_id, "🎬 لینک ویدیو برای ضبط را بفرستید:")
    elif data == "menu_settings":
        result = send_message(chat_id, "⚙️ تنظیمات:", reply_markup=settings_keyboard(session.settings))
        if result and "message_id" in result:
            session.last_settings_msg_id = result["message_id"]
            set_session(session)
    ...
    elif data in ("set_dlmode", "set_brwmode", "set_deep", "set_recbeh", "set_audio", "set_vfmt", "set_incognito", "set_viddel"):
    ...
    # ★ callbackهای مربوط به انتخاب ZIP/اصلی برای ضبط حذف شده‌اند
...
def main():
    ...
    safe_print("✅ Bot21 PulseAudio Pro اجرا شد")
    try:
